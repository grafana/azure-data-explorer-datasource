...
test.describe('Azure Data Explorer queries - smoke', () => {
  test('renders KQL editor', async ({ panelEditPage, page, grafanaVersion }) => {
    ...
    if (versionValue) {
      await expect(
        queryEditorRow(page).getByTestId(selectors.components.queryEditor.cluster.input.selector)
      ).toBeVisible();
    } else {
      // data-testid was not passed to the select component prior to 11.1.0
      await expect(queryEditorRow(page).getByText('Cluster')).toBeVisible();
    }
    if (versionValue) {
      await expect(
        queryEditorRow(page).getByTestId(selectors.components.queryEditor.database.input.selector)
      ).toBeVisible();
    } else {
      // data-testid was not passed to the select component prior to 11.1.0
      await expect(queryEditorRow(page).getByText('Database')).toBeVisible();
    }

    await page.getByText('KQL').click({ force: true });
    await expect(
      queryEditorRow(page).getByTestId(selectors.components.queryEditor.codeEditor.container)
    ).toBeVisible();
  });
