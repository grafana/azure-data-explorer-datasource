  ...
  const authTypeOptions = useMemo<Array<SelectableValue<AzureAuthType>>>(() => {
    ...
    if (props.oboEnabled) {
      opts.push({
        ...
        label: t("components.azure-credentials-form.auth-type-options.label.app-registration-on-behalf-of", "App Registration (On-Behalf-Of)"),
      });
    }
    ...
    <ConfigSection title={t("components.azure-credentials-form.title-authentication", "Authentication")}>
      {authTypeOptions.length > 1 && (
        <Field
          ...
          description={t("components.azure-credentials-form.description-choose-authentication-azure-services", "Choose the type of authentication to Azure services")}
      ...
      {credentials.authType === 'currentuser' && (
        <>
          <Alert title={t("components.azure-credentials-form.title-current-user-authentication-is-experimental", "Current user authentication is experimental")} severity="warning">
      ...
      {credentials.authType === 'clientsecret-obo' && (
        <>
          <Alert title={t("components.azure-credentials-form.title-on-behalf-of-feature-is-in-beta", "On-Behalf-Of feature is in beta")} severity="warning">
      ...
      {(credentials.authType === 'clientsecret' || credentials.authType === 'clientsecret-obo') && (
        <>
          ...
          {typeof credentials.clientSecret === 'symbol' ? (
            <Field label={t("components.azure-credentials-form.label-client-secret", "Client Secret")} htmlFor="aad-client-secret-configured" required>
              <div className="width-30" style={{ display: 'flex', gap: '4px' }}>
                <Input
                  id="aad-client-secret-configured"
                  aria-label={t("components.azure-credentials-form.aad-client-secret-configured-aria-label-client-secret", "Client Secret")}
                  placeholder={t("components.azure-credentials-form.aad-client-secret-configured-placeholder-configured", "configured")}
                  disabled={true}
                />
                <Button variant="secondary" type="button" onClick={onClientSecretReset}><Trans i18nKey="components.azure-credentials-form.reset">
                  Reset
                </Trans></Button>
              </div>
            </Field>
